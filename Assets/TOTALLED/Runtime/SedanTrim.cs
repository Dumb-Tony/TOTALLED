...
namespace Totalled
{
    ...
    {
        sealed class Patch {public int[] nodes;public Vector4 rect;public Mesh mesh;public GameObject go;public bool torn;public float maxSpan;}
        ...
        public SedanTrim(SacrificialSedan car,Transform parent)
        {
            ...
            foreach(var end in new[]{front,back})
            {
                ...
                for(int line=0;line<4;line++)Add(end,new Vector4(.32f,.39f+line*.10f,.68f,.41f+line*.10f),chrome,"Grille bar");
            ...
            for(int side=0;side<2;side++)
            {
                ...
                for(int z=0;z<6;z++)Add(new[]{I(x,0,z),I(x,0,z+1),I(x,1,z+1),I(x,1,z)},new Vector4(0,.08f,1,.18f),black,"Sill trim");
            }
        }
        ...
        void Add(int[] nodes,Vector4 rect,Material mat,string name)
        {
            ...
            patches.Add(new Patch{nodes=nodes,rect=rect,go=go,mesh=mesh,maxSpan=span*1.6f});
        }
