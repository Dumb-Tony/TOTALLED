...
namespace Totalled
{
    public sealed class CrashLab : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if(Input.GetKeyDown(KeyCode.Tab))debugMode=(debugMode+1)%4;
            ...
            if(Input.GetKeyDown(KeyCode.Alpha5))Impact(4);
            if(Input.GetKeyDown(KeyCode.Alpha6))Impact(5);
        ...
        void OnGUI()
        {
            ...
            if(!telemetry)
            {
                ...
                GUI.Label(new Rect(32,58,240,23),"MOTOR WORKS  /  CRASH LAB 0.8.1",smallStyle);
            ...
            GUILayout.BeginArea(new Rect(32,27,335,465));GUILayout.Label("TOTALLED",titleStyle);GUILayout.Label("CRASH LAB  /  SACRIFICIAL SEDAN  /  0.8.1",smallStyle);GUILayout.Space(12);
            ...
            GUILayout.Label($"TAB stress / plastic / broken / zones  •  C components  •  RMB orbit / scroll zoom",smallStyle);
            GUILayout.Label("5 parked target / 6 sideways • H hide telemetry • Both cars deform",smallStyle);
            GUILayout.EndArea();
            if(debug) { GUI.Label(new Rect(395,25,500,30),"STRUCTURE / "+new[]{"STRESS","PLASTIC TRAVEL","BROKEN CONNECTIONS","CRUMPLE ZONES"}[debugMode],textStyle); }
        }
