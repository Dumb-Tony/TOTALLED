...
`SacrificialSedan` defines a triangulated 42-node base lattice plus twelve physical front/rear fold nodes, four roof nodes, four ten-node panels, and four wheel hub nodes. The dense cabin is stronger than the ends. Each end has softer outer members, stronger inner rails and a transition into the cabin; shorter-member yield is length-normalized. Subdivision preserves total mass at 1272 kg. Each panel has a braced 3×3 surface and one internal support node. Two stronger hinge mounts and a weaker latch each use a triangulated group of constraints that fail together. Surface subdivisions permit localized creases. Yielded beam impulses are bounded and plastic rest lengths flow toward actual deformation without overshooting. Wheel hubs have four suspension links, with a more compliant spring link. Rear hubs receive drive force. The steering rack is rate-limited with speed-sensitive lock and approximate Ackermann angles. Opposite throttle brakes before reversing; longitudinal and lateral forces share a friction budget. The service brake acts on all four wheels; the handbrake progressively locks the rear pair and reduces rear lateral grip, preserving front steering traction. Current mount vectors determine steering and tire force axes. Component condition records maximum local span distortion; cooling temperature and fuel leakage evolve in time.

`SedanView` renders surfaces directly from current node positions, with original procedural placeholder geometry. Panels are their own deforming surfaces. Debug modes show nodes, stress, accumulated plastic travel, broken connections, crumple-zone membership and recent contact impulses. `CrashLab` owns the environment, repeat impact launch commands, recovery, time controls, follow/orbit camera, telemetry and new specimens.
...
6. **Visuals:** procedural tapered body with sloped transparent glass, trim and lamp lenses, rounded rotating tires, visible seats/dashboard and a filled engine bay. Cabin and engine geometry skin to chassis nodes; they are not independently colliding or detachable components. Visual offsets do not reshape the collision graph. Wheel tubs are visual only. Frame distortion removes glass cosmetically; shards and particles are not simulated. Synthesized engine/impact/tire audio and bounded persistent skid marks observe the physical simulation. Make deformation convincing before producing art.
