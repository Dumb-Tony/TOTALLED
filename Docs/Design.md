...
`SacrificialSedan` defines a triangulated 42-node lower lattice, four roof nodes, four four-node panels, and four wheel hub nodes. The dense cabin is stronger than the ends. Panels have two stronger hinge mounts and a weaker latch. Wheel hubs have four suspension links, with a more compliant spring link. Rear hubs receive drive force. Current mount vectors determine steering and tire force axes. Component condition records maximum local span distortion; cooling temperature and fuel leakage evolve in time.
